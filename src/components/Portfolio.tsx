...
export default function Portfolio() {
  ...
  const locale = useLocale(); // Grab the current active language (en or fr)

  useEffect(() => {
    async function fetchProjects() {
      setIsLoading(true); // Ensure it shows loading when switching languages
      try {
        // We append the dynamic locale to the Payload CMS API request!
        const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/projects?depth=1&locale=${locale}`);
    ...
    fetchProjects();
  }, [locale]); // The effect re-runs whenever the user toggles the language
